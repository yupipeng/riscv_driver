...
void _premain_init(void)
{
    ...
    if (hartid == BOOT_HARTID) { // only required for boot hartid
        ...
        //* sdk init start
        extern void hw_cpu_clock_init(void);
        #ifdef REG_GPIO0_DIN
        if (REG_GPIO0_DIN & (0x1<<9))
        #else
        if ((*(volatile unsigned int *)(0x18700000 + 0)) & (0x1<<9))
        #endif
        {
            while (1) {
                __asm__("nop");
        ...
        hw_cpu_clock_init();
        hw_uart_setup(UART0, 115200, UART_PARITY_NON, 1, 1);
        
        
        SystemBannerPrint(); //* this stupid function call printf before main, cause a lot of problem
