...
/** @brief Helper macro for calling inv_msg() while redefining printer function
 *	If INV_MSG_DISABLE compile switch is set for a compilation unit
 *	messages will be totally disabled
 */
#define INV_MSG_P(level, printer, ...) _INV_MSG_P(level, printer, __VA_ARGS__)
 ...
 *  Function uses stdio. Care must be taken on embedded platform.
...
/** @brief Display a message (through means of new printer function)
 *  @param[in]	level    for the message
 *  @param[in]	printer  user provided function in charge printing message
 *  @param[in]	str      message string
 *  @param[in]	...      optional arguments
