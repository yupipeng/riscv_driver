...
static void app_task_timer_tick_set(U32 timer_id, U32 tick)
{
	...
	{
		printf("timer id is not correct\r\n");
	}
}
				...
				if (diff_tick <= 0)
				{
					...
					switch (timer_id)
					{
						...
						case TIMER_CombKey:					
							...
							{
								printf("combo Abort cause by window change\r\n");
								key_process_envs.processing_key_status = KEY_RELEASED;
								break;
							}
			...
			case enTYPE_KEY:	
				...
				case KEY_PRESSED:
					...
					{						
						...
						{
							//a key end
							printf("key Abort caused by window change\r\n");
							key_process_envs.processing_key_status = KEY_RELEASED;
							break;
						}
				...
				case KEY_WAIT_COMB_KEY:
					...
					else
					{
						// a new key in
						key_process_envs.key_param.combo_end = 1;
						printf("combo Abort cause by new key\r\n");
...
void func_wo_param_msgSend ( pFUNCTION func )
{
	...
	{
		printf("app func task queue is full\r\n");
		DBG_Assert(FALSE);
	}
}
